...
VIEW.map = function(v){
  ...
  CAMPUS.forEach(function(c){
    ...
    GANS.filter(function(g){ return g.campus === c; }).forEach(function(g){
      inner.appendChild(h("label", { "class": "check", html:
        '<input type="checkbox" data-gid="' + g.id + '" checked> ' + g.name }));
    });
    box.appendChild(inner); list.appendChild(box);
  });
  var stage = h("div", { id: "map-stage",
    style: "height:420px;border-radius:16px;margin-top:14px;display:flex;align-items:center;" +
           "justify-content:center;text-align:center;background:var(--surface-2);border:1px solid var(--border)" },
  ...
  v.appendChild(h("div", { "class": "panel" }, [
    ...
    toolbar(["⚡ שיבוץ אוטומטי לפי קרבה", "📤 ייצוא"]),
    h("details", { "class": "map-fold", open: "" }, [
      h("summary", { text: "סינון וגנים להצגה" }),
      h("div", { "class": "map-fold-body" }, [ list ])
    ]),
    stage
  ]));
};
...
window.__uiLab = Object.freeze({
  ...
  subtitle: function(tab){
    ...
    switch(tab){
      ...
      case "map":          return YEAR + " · " + T.gans + " גנים";
