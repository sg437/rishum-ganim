...
**דוגמה — עמוד הבית של המשרד הראשי (ברמה ארצית):**
בדיוק כמו עמוד הבית שכבר בנינו לעיר, אבל מסכם את **כל הארץ** ומתעדכן **אונליין**:
- **סה"כ ילדות בכל הארץ, מחולק לפי חינוך** (רגיל / מיוחד).
- **סה"כ גנים** בכל הארץ (ואפשר גם לפי חינוך).
- **ממוצע ילדות לגן — כללי** (מכל הערים יחד).
- ומעליהם בורר הערים — כך שאותם מספרים יכולים להיות "כל הארץ" או תת-קבוצה נבחרת.

> ב-SQL זה שאילתת סיכום פשוטה (`count`, `avg`) על פני כל הערים המורשות, מוגשת ל-`VIEW` ומתרעננת בזמן אמת
> (Realtime). למשל: `select education, count(*) from students where not finished group by education`.
